...
public class Zip extends CordovaPlugin {
    ...
    private void unzipSync(CordovaArgs args, CallbackContext callbackContext) {
        ...
        try {
            ...
            ArchiveInputStream ais = new ZipArchiveInputStream(inputStream, "UTF-8", false, true);
            inputStream = ais;
            ...
            while ((ze = (ZipArchiveEntry) ais.getNextEntry()) != null)
            {
                anyEntries = true;
    ...
    private void unzipSyncOld(CordovaArgs args, CallbackContext callbackContext) {
        InputStream inputStream = null;
        try {
            String zipFileName = args.getString(0);
            String outputDirectory = args.getString(1);
            ...
            // The inputstream is now pointing at the start of the actual zip file content.
            ZipArchiveInputStream ais = new ZipArchiveInputStream(inputStream, "UTF-8", false, true);
            inputStream = ais;
            ...
            while ((ze = (ZipArchiveEntry) ais.getNextEntry()) != null)
            {
                ...
                if (ze.isDirectory()) {
                   ...
                   dir.mkdirs();
                    ...
                    file.getParentFile().mkdirs();
                    if(file.exists() || file.createNewFile()){
                        Log.w("Zip", "extracting123: " + file.getPath());
                        FileOutputStream fout = new FileOutputStream(file);
                        int count;
                        while ((count = ais.read(buffer)) != -1)
                        {
